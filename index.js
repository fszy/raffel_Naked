...
/* Modify lines below */
var catchall = "flqmail.com"; // For the accounts (if you win)
var password = 'urown'; // For the accounts (if you win)
var retryDelay = 1500; // How many miliseconds before retrying if error
var tasks = 10; //Change this to the amount of accounts you would like entered
/* End */
...
function getRaffleToken(taskNum, proxy, request, email, firstName, lastName) {
	...
	request({
		url: 'https://nakedcph.typeform.com/app/form/result/token/UQMUMl/default',
		method: 'get',
		headers: {
			'User-Agent': userAgent,
		},
...
function submitRaffle(taskNum, proxy, request, email, firstName, lastName, raffleToken) {
	request({
		url: 'https://nakedcph.typeform.com/app/form/submit/UQMUMl',
		method: 'POST',
		headers: {
			'x-requested-with': 'XMLHttpRequest',
			'User-Agent': userAgent,
		...
		formData: {
			'form[language]': 'en',
			'form[textfield:sPnevWyXjk1E]': firstName,
			'form[textfield:RIN84vyYuCuW]': lastName,
			'form[email:SGdUT5ZzfDt6]': email,
			'form[dropdown:FuG7bj4Yq3aH]': 'Denmark',
